...
/// Render `text` by stamping filled glyphs starting at `(x, y)`.
/// `y` is the top of the glyph cell (8 px tall).
pub fn fill_text(
    buf: &mut Vec<u8>,
    width: u32,
    height: u32,
    ...
    for ch in text.chars() {
        ...
        for (row, &byte) in glyph.iter().enumerate() {
            for col in 0..8i64 {
                if byte & (0x80 >> col) != 0 {
                    put_pixel(buf, width, height, cx + col, cy + row as i64, color, clip);
                }
            }
        }
        cx += 8;
...
/// Render `text` as outlined glyphs (stroke each set pixel with a thin cross).
/// For simplicity, this draws the same bitmap as `fill_text` but with the
/// stroke colour; for lineWidth > 1 each glyph pixel is expanded.
pub fn stroke_text(
    buf: &mut Vec<u8>,
    width: u32,
    height: u32,
    ...
    let hw = (line_width / 2.0).max(0.5).ceil() as i64;
    let mut cx = x.round() as i64;
    let cy = y.round() as i64;
    for ch in text.chars() {
        let glyph = font::glyph(ch);
        for (row, &byte) in glyph.iter().enumerate() {
            for col in 0..8i64 {
                if byte & (0x80 >> col) != 0 {
                    // Expand each set pixel by hw in every direction.
                    for dy in -hw..=hw {
                        for dx in -hw..=hw {
                            put_pixel(
                                buf, width, height,
                                cx + col + dx, cy + row as i64 + dy,
                                color, clip,
                            );
                        }
                    }
